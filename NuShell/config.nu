...
export def --env xenv [] {
    if not (".xenv" | path exists) {
        print $"(ansi yellow)===>>  .xenv not found.  <<==="
        return
    }
    ...
    let valid_paths = (
        ...
        | where { |p| $p != "" }
        | each { |pa|
			let p = ($pa | str replace --all "/" "\\")
            if ($p | path exists) {
                print $"(ansi green)Path Loaded:(ansi reset)    ($p)"
                $p
            } else {
                print $"(ansi yellow)Path Not Found:(ansi reset) ($p)"
                null
            }
        }
        | where { |p| $p != null }
    ...
    $env.PATH = ($env.PATH | append $valid_paths | uniq)

    print $"(ansi b)===>>  .xenv loaded!  <<==="
}
